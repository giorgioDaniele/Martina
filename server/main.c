...
static int
client_authentication (struct sockaddr_in sin, int cs) {
...
io_err:
    fprintf(stdout, "[-] Couldn't open registered users file\n");
    ZERO(&msg, sizeof(struct Message));
    msg.cmd = FAILURE;
    write (cs, (const void*) &msg, sizeof(msg.cmd) + strlen(msg.data) + 1);
    return 0;
...
static int 
init_fds (int * css, int max, fd_set * readfds) {
...
static void 
handle_tun (int tun, char *net, char *netmask) {
...
main (int argc, char **argv) {
    ...
    if((tun = tun_alloc(dev)) < 0)
        goto tun_error;
    if((res = set_addr(dev, "10.0.0.11", 24)) < 0)
    ...
    len = sizeof (cin);
    ZERO(css, MAX_CLIENTS * sizeof(int));
    ...
    for(;;) {
        ...
        if (FD_ISSET(ss, &readfds)) {
            ...
            if(client_authentication(cin, cs) <= 0) {
                fprintf(stdout, "   [+] End connection:");
                fprintf(stdout, " - Addr: %s", inet_ntoa (cin.sin_addr));
                fprintf(stdout, " - Port: %u\n", ntohs(cin.sin_port));
                close(cs);
            ...
            handle_tun(tun, net, netmask);
        } else {
            for (i=0; i<MAX_CLIENTS; i++) {
                cs = css[i];
                if (FD_ISSET(cs, &readfds)) 
                    handle_cs(&css[i], cin, net, netmask, tun);
            }
        }
    }
